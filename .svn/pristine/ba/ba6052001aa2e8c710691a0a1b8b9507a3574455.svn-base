...
{
    /* check duplicate */
    symbol_t* tmp = find_symbol(name);
    if (tmp)return -1;
    /* create new symbol_t (don't forget to free it)*/
    tmp = malloc(sizeof(symbol_t));//don't use new!
    tmp->name = name;
    tmp->addr = vmaddr;
...
//    reltab->next = tmp;
//}
// reltab is not only a pointer,see init():reltab = (reloc_t *)malloc(sizeof(reloc_t)); // free in finit
...
parse_t parse_digit(char **ptr, long *value)
{
    ...
    *value = strtoull(*ptr, ptr, 0);
    // *value = sign*strtoll(*ptr, ptr, 0);
    return PARSE_DIGIT;

}
//strtoll:
//the first parameter is the string to be converted
//The second parameter can be empty,if it is not empty,endptr saves the address of the 
//illegal character before the end of the function
...
parse_t parse_imm(char **ptr, char **name, long *value)
{
    /* skip the blank and check */
    SKIP_BLANK(*ptr);
    if (!IS_IMM(*ptr)&&!IS_LETTER(*ptr))return PARSE_ERR;
...
//the functions have been defined above:
//parse_instr
//parse_delim
//parse_reg
//parse_symbol
...
*           call SUM  #invoke SUM function */
    ...
    if (parse_label(&ptr,&name) == PARSE_LABEL) {
        ...
        line->y64bin.bytes = 0;
        if (add_symbol(name) == -1) {       //repeadly use of a same label
            line->type = TYPE_ERR;
    ...
    case I_POPQ:
    case I_PUSHQ:
        if (parse_reg(&ptr, &regidA) == PARSE_ERR) {
            line->type = TYPE_ERR;
    ...
    case I_ALU:
        ...
        if (parse_reg(&ptr, &regidB) == PARSE_ERR) {
    ...
    case I_JMP:
    case I_CALL:
        parse_t_tmp = parse_imm(&ptr, &name, &value);
        if (parse_t_tmp == PARSE_ERR) {
            line->type = TYPE_ERR;
            err_print("Invalid DEST");
            return TYPE_ERR;
        }
        else if (parse_t_tmp == PARSE_DIGIT)
            for (int i = 0; i < 8; i++)
    ...
    case I_IRMOVQ:
        ...
        if (parse_t_tmp == PARSE_DIGIT) 
            for (int i = 0; i < 8; i++)line->y64bin.codes[i + 2] = (value >> (8 * i)) & 0xFF;
        if (parse_t_tmp == PARSE_SYMBOL)
            add_reloc(name, &line->y64bin);
        break;
    case I_RMMOVQ:
        if (parse_reg(&ptr, &regidA) == PARSE_ERR) {
        ...
        if(parse_mem(&ptr,&value,&regidB)==PARSE_ERR) {
            line->type = TYPE_ERR;
    ...
    case I_DIRECTIVE:
        ...
        case D_DATA:
            parse_t_tmp = parse_data(&ptr, &name, &value);
            if (parse_t_tmp == PARSE_ERR) {
                line->type = TYPE_ERR;
                return TYPE_ERR;
            }
            else if (parse_t_tmp == PARSE_DIGIT)
                for (int i = 0; i < line->y64bin.bytes; i++)line->y64bin.codes[i] = ((value >> (8 * i)) & 0xFF);
            else
                add_reloc(name, &line->y64bin);
            break;
    ...
    while (rtmp) {
        ...
        case 8://D_DATA
            for (int i = 0; i < rtmp->y64bin->bytes; i++)
                rtmp->y64bin->codes[i] = ((symtmp->addr) >> (8 * i))& 0xFF;
            break;
        case 9://label
            for (int i = 0; i < 8; i++)
                rtmp->y64bin->codes[1+i] = ((symtmp->addr) >> (8 * i)) & 0xFF;
            break;
        case 10:
            for (int i = 0; i < 8; i++)
                rtmp->y64bin->codes[2 + i] = ((symtmp->addr) >> (8 * i)) & 0xFF;
            break;
        default:
            return -1;
        }
...
int binfile(FILE *out)
{
    ...
    line_t* line_current = line_head;
    /* binary write y64 code to output file (NOTE: see fwrite()) */
    while (line_current)
    {
        if (line_current->type == TYPE_INS){
            fseek(out, line_current->y64bin.addr, SEEK_SET);//set offset,very important!!!
            fwrite(line_current->y64bin.codes, 1, line_current->y64bin.bytes, out);
        }
