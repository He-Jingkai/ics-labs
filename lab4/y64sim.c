...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    switch (op) {
    case A_ADD:
        if (argA > 0 && argB > 0 && val < 0)ovf = TRUE;
        if (argA < 0 && argB < 0 && val > 0)ovf = TRUE;
        break;
    case A_SUB:
        if (argB > 0 && argA < 0 && val < 0)ovf = TRUE;
        if (argB < 0 && argA > 0 && val > 0)ovf = TRUE;
        break;
    default:
    	break;
...
bool_t cond_doit(cc_t cc, cond_t cond) 
{
    ...
    case C_YES:
        ...
        break;
    case C_L:
        doit = (!GET_OF(cc) & GET_SF(cc))|(GET_OF(cc) & !GET_SF(cc));
        break;
    case C_E:
        doit = GET_ZF(cc);
        break;
    case C_LE:
        doit = (GET_OF(cc) ^ GET_SF(cc))|GET_ZF(cc);
        break;
    case C_NE:
        doit = !GET_ZF(cc);
        break;
    case C_G:
        doit = !(GET_OF(cc) ^ GET_SF(cc))&!GET_ZF(cc);//LE bar
        break;
    case C_GE:
        doit = !(GET_OF(cc) ^ GET_SF(cc));
        break;
   }
    return doit;
}
 ...
 * I_HALT , I_NOP, I_RRMOVQ, I_IRMOVQ, I_RMMOVQ, I_MRMOVQ,I_ALU, I_JMP, I_CALL, I_RET, I_PUSHQ, I_POPQ, I_DIRECTIVE
 * return
 *     STAT_AOK: continue
 *     STAT_HLT: halt
...
stat_t nexti(y64sim_t *sim)
{
    ...
    switch (icode) {
      ...
      case I_RMMOVQ: /* 4:0 regA:regB imm */
          if (!set_long_val(sim->m, immediate + get_reg_val(sim->r, regB), get_reg_val(sim->r, regA))) {
              err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, immediate + get_reg_val(sim->r, regB));
              return STAT_INS;
          }
              sim->pc = next_pc;
              break;
      ...
      case I_MRMOVQ: ;/* 5:0 regB:regA imm */
          long_t tmp_des;
          if (!get_long_val(sim->m, get_reg_val(sim->r, regB) + immediate, &tmp_des))
          {
              err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, immediate + get_reg_val(sim->r, regB));
              return STAT_ADR;
          }
          set_reg_val(sim->r, regA, tmp_des);
           sim->pc = next_pc;
          break;

      case I_ALU: ;/* 6:x regA:regB */  
          long_t reg_a=get_reg_val(sim->r, regA);
          long_t reg_b=get_reg_val(sim->r, regB);
          long_t val_tmp = compute_alu(ifun, reg_a, reg_b);
          set_reg_val(sim->r, regB, val_tmp);
          sim->cc = compute_cc(ifun, reg_a, reg_b, val_tmp);
          sim->pc = next_pc;
          break;
      ...
      case I_CALL: ;/* 8:x imm */
          ...
          if (!set_long_val(sim->m, rsp, next_pc))//next_pc, not sim->pc!!!
          {
              err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, rsp);
              return STAT_ADR;
          }
          //set_reg_val(sim->r, REG_RSP, rsp - 0x8);
          sim->pc = immediate;
          break;
          
       case I_RET:; /* 9:0 */
           long_t rsp2 = get_reg_val(sim->r, REG_RSP);
           long_t to_rega;
           set_reg_val(sim->r, REG_RSP, rsp2 + 0x8);
           if (!get_long_val(sim->m, rsp2, &to_rega))
           {
               err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, rsp2);
               return STAT_INS;
           }
           //set_reg_val(sim->r, REG_RSP, rsp2 + 0x8);
           sim->pc = to_rega;
      ...
      case I_PUSHQ:; /* A:0 regA:F */
          long_t rsp3=get_reg_val(sim->r, REG_RSP)-8;
          long_t regaa=get_reg_val(sim->r, regA);//??
          //set_reg_val(sim->r, REG_RSP, rsp3);//why???
          if (!set_long_val(sim->m, rsp3, regaa))
          {
              err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, rsp3);
              set_reg_val(sim->r, REG_RSP, rsp3 );
              return STAT_ADR;
          }
          set_reg_val(sim->r, REG_RSP, rsp3 );
          sim->pc = next_pc;
...
// why in pushq and call,set_reg_val(sim->r,regA) in front of set_long_val
// it's right when: long_t rega=get_reg_val(sim->r, regA) ; set_long_val(sim->m, rsp3, rega) ;
// but it's wrong when:set_long_val(sim->m, rsp3, get_reg_val(sim->r, regA));


      case I_POPQ: ;/* B:0 regA:F */
          long_t rsp4 = get_reg_val(sim->r, REG_RSP);
          long_t to_rega1;
          set_reg_val(sim->r, REG_RSP, rsp4 + 0x8);//
          if (!get_long_val(sim->m, rsp4, &to_rega1))
          {
              err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, rsp4);
              return STAT_INS;
          }
          //set_reg_val(sim->r, REG_RSP, rsp4 + 0x8);
          set_reg_val(sim->r, regA, to_rega1);
